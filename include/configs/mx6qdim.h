#ifndef _MX6QDIM_CONFIG_H
#define _MX6QDIM_CONFIG_H
...
//#define CONFIG_PREBOOT "run env_load"
#define CONFIG_EXTRA_ENV_SETTINGS \
	"bootcmd_android=run env_load_android; " \
		"booti mmc${boot_dev}\0" \
	"bootcmd_android_recovery=run env_load_android; " \
		"booti mmc${boot_dev} recovery\0" \
	"bootcmd_linux=run env_load_linux; " \
		"run linux_load; " \
		"bootz ${loadaddr} ${initrd_addr} ${fdt_addr}\0" \
	"bootcmd_mfg=bootz ${loadaddr} ${initrd_addr} ${fdt_addr}\0" \
	"env_load_android=if ext2load mmc ${env_dev}:${env_part} ${loadaddr} ${env_file}; then " \
		"env import -t ${loadaddr} ${filesize}; fi\0" \
	"env_load_linux=if fatload mmc ${env_dev}:${env_part} ${loadaddr} ${env_file}; then " \
		"env import -t ${loadaddr} ${filesize}; fi\0" \
	"fdt_addr=0x14f00000\0" \
	"fdt_high=0xffffffff\0" \
	"initrd_addr=0x20000000\0" \
	"initrd_high=0xffffffff\0" \
	"linux_fdt=/imx6q-dim.dtb\0" \
	"linux_initrd=/rootfs.cpio.uboot\0" \
	"linux_kernel=/zImage\0" \
	"linux_load=fatload mmc ${boot_dev}:${boot_part} ${loadaddr} ${linux_kernel}; " \
		"fatload mmc ${boot_dev}:${boot_part} ${initrd_addr} ${linux_initrd}; " \
		"fatload mmc ${boot_dev}:${boot_part} ${fdt_addr} ${linux_fdt}\0"
...
#define CONFIG_FASTBOOT_TRANSFER_BUF_SIZE	0x70000000

#if defined(CONFIG_GCU)
#define CONFIG_DIM_BCM89810
